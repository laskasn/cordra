...
public class TransactionFileLogger {
    ...
    public static void main(String[] args) throws Exception {
        ...
        TransactionFileLogger logger = new TransactionFileLogger(path, groupId, kafkaBootstrapServers, propsAsMap);
        logger.start();
    ...
    public TransactionFileLogger(Path dir, String groupId, String kafkaBootstrapServers, Map<String, String> props) {
        // currently needs to be single-threaded due to setting of PrintWriter pw
    ...
    static DateTimeFormatter yearMonthDayFormatter = DateTimeFormatter.ofPattern("uuuuMMdd").withZone(ZoneOffset.UTC);
    ...
    public void consume(String message, long timestamp) {
        ...
        try {
            ensurePrintWriterForTimestamp(timestamp);
        } catch (IOException e) {
    ...
    public void ensurePrintWriterForTimestamp(long timestamp) throws IOException {
        String newFilename = getFilenameForTimestamp(timestamp);
        if (currentFilename == null) {
            setPrintWriterFor(newFilename);
        } else if (currentFilename.compareTo(newFilename) < 0) {
            pw.close();
            setPrintWriterFor(newFilename);
        }
    }

    private void setPrintWriterFor(String filename) throws IOException {
        pw = new PrintWriter(Files.newBufferedWriter(dir.resolve(filename), StandardOpenOption.WRITE, StandardOpenOption.CREATE, StandardOpenOption.APPEND));
