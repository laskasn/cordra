...
/**
 * HeaderSet objects manage, parse, and format a set of key-value pairs that are
 * used with DOConnection messages.
 */
public class HeaderSet implements Iterable<HeaderItem> {
    ...
    /**
     * This method will populate the set of headers with data from the given
     * InputStream. The stream will be read using the UTF8 character encoding.
     * Returns false if a newline or EOF was reached before any headers were
     * found.
     */
    public boolean readHeaders(InputStream in) throws IOException {
        headers.clear();
        ...
        while (index < bytes.length) {
            ...
            if (b == '\n' || b == '&') {
                if (scanningValue) {
                    String value;
                    if (sawPercent)
                        value = unescapeURLTxt(bytes, start, index - start);
                    else
                        value = Util.decodeString(bytes, start, index - start);
                    HeaderItem newItem = new HeaderItem(key, value);
                    headers.add(newItem);
                    scanningValue = false;
                } else {
                    if (sawPercent)
                        key = unescapeURLTxt(bytes, start, index - start);
                    else
                        key = Util.decodeString(bytes, start, index - start);
    ...
    public boolean readHeadersFromString(String str) {
        ...
        while (endIdx >= 0) {
            ...
            String segment = endIdx < 0 ? str.substring(startIdx + 1) : str.substring(startIdx + 1, endIdx);
    ...
    public static final void writeEscapedTxt(OutputStream out, String str) throws IOException {
        byte[] utf8Buf = str.getBytes(StandardCharsets.UTF_8);
        byte ch;
    ...
    public static final byte decodeHexByte(char ch1, char ch2) {
        ch1 = (char) ((ch1 >= '0' && ch1 <= '9') ? ch1 - '0'
                : ((ch1 >= 'a' && ch1 <= 'z') ? ch1 - 'a' + 10 : ch1 - 'A' + 10));
        ch2 = (char) ((ch2 >= '0' && ch2 <= '9') ? ch2 - '0'
                : ((ch2 >= 'a' && ch2 <= 'z') ? ch2 - 'a' + 10 : ch2 - 'A' + 10));
        return (byte) (ch1 << 4 | ch2);
    }

    public static final byte decodeHexByte(byte ch1, byte ch2) {
        int n1 = ((ch1 >= '0' && ch1 <= '9') ? ch1 - '0'
                : ((ch1 >= 'a' && ch1 <= 'z') ? ch1 - 'a' + 10 : ch1 - 'A' + 10));
        int n2 = ((ch2 >= '0' && ch2 <= '9') ? ch2 - '0'
                : ((ch2 >= 'a' && ch2 <= 'z') ? ch2 - 'a' + 10 : ch2 - 'A' + 10));
        return (byte) (n1 << 4 | n2);
    }

    /**
     * Returns a new HeaderSet with the key-value pairs for which the keys have
     * the given prefix. This adds all of the key-value pairs, with the prefix
     * removed from the keys, to a new HeaderSet and returns it.
     */
    ...
    /**
     * Returns the integer value for the last header with the given name. If no
     * header with the given name is found or if there is a formatting error,
    ...
    /**
     * Returns the long (64 bit integer) value for the last header with the
     * given name. If no header with the given name is found or if there is a
     * formatting error, defaultValue is returned.
     */
    ...
    /**
     * Returns the boolean value for the last header with the given name. If no
     * header with the given name is found or if there is a formatting error,
     * defaultValue is returned.
     */
    public boolean getBooleanHeader(String headerName, boolean defaultValue) {
        for (int i = headers.size() - 1; i >= 0; i--) {
            ...
            if (hdr.name.equalsIgnoreCase(headerName)) {
                if (hdr.value == null)
                    return defaultValue;
                if (hdr.value.equals("1") || hdr.value.equals("true") || hdr.value.equals("yes"))
                    return true;
                else if (hdr.value.equals("0") || hdr.value.equals("false") || hdr.value.equals("no"))
                    return false;
                else
                    return defaultValue;
            }
    ...
    /**
     * Returns the String value for the last header with the given name. If no
     * header with the given name is found, defaultValue is returned.
     */
    ...
    /**
     * Returns the byte array value for the last header with the given name. If
     * no header with the given name is found, defaultValue is returned.
     */
