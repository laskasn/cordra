function DetailedObjectViewer(containerDiv, contentPlusMeta) {
    ...
    function constructor() {
        var closeButton = $(
            '<button class="btn btn-sm btn-default">Close</button>'
        );
        containerDiv.append(closeButton);
        closeButton.on("click", onCloseClick);

        var digitalObjectDiv = $(
            '<div class="well well-sm" style="background-color: #eaeaea"></div>'
        );
    ...
    function buildPayloadViewers(payloads, digitalObjectDiv) {
        ...
        for (var i = 0; i < payloads.length; i++) {
            ...
            var payloadDiv = $(
                '<div class="media well well-sm" style="background-color: #f5f5f5"></div>'
            );
