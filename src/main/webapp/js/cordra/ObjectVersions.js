function ObjectVersions(containerDiv, versions, objectId, allowEdits) {
    ...
    function constructor() {
        ...
        if (allowEdits) {
            ...
            publishButton.on("click", onPublishClick);

            var publishButtonSpan = $("<span></span>");
            publishButton.append(publishButtonSpan);
            publishButtonSpan.text("Make Version");
        }
        versionsListDiv = $('<div class="col-md-12"></div>');
        ...
        if (versionsList.length == 0 || versionsList.length == 1) {
            ...
            for (var i = 0; i < versionsList.length; i++) {
                ...
                var link = $('<a style="display:inline-block" target="_blank">')
                    .attr("href", "objects/" + versionInfo.id)
                    .text(versionInfo.id);
    ...
    function publishVersion() {
        APP.publishVersion(
            objectId,
            onPublishVersionSuccess,
            onPublishVersionError
        );
    }
